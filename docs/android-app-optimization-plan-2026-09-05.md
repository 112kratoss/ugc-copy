...
Store, 2026-09-05 evening: PR #112 squash-merged as `main` 62e9f57 (Quality and the web Production release green, production reports the commit); `mobile-store-release.yml` run 33969667328 built EAS 21641ad1 = 0.1.4 (71), fingerprint `db146ca3…` (identical to the local computation), and submitted it to the closed alpha at 14:07 UTC. Play bundle explorer on 71: **App optimization High** (70 was Medium), obfuscation 80 %, optimization and shrinking percentages still "–" (Play did not compute them for the AGP 8.12 bundle either time), R8 checklist green on Full Mode, Resource Shrinking and Resource Shrinking Optimized, outstanding: Repackage Classes and AGP 9; install size 20.4 MB (−3.27 MB vs 70), update size 5.38 MB, uncompressed dex 12.7 MB, 19,270 supported devices unchanged. A production release 0.1.4 (71) was created from the library (the alpha release was still in review, so Promote was unavailable), release notes set, **full rollout at the owner's request** instead of the staged 20 % this plan recommended, and sent for review, restarting the alpha review. `ota-targets.json` records 71 in the same PR as this paragraph.

Still open: Google's review of the production release (managed publishing is off, so approval publishes it), 5d renditions, phase 6 with the AGP 9 Expo SDK, and iOS OTA parity (main's iOS fingerprint moved with the plugin edit; the next iOS binary restores it).
...
- **Phone smoke — 2026-09-05 18:00–18:15, Galaxy S24 Ultra SM-S928B (Android 16, 1440×3120 at 600 dpi), the phase 5 debug-signed APK installed after uninstalling Play's 0.1.4 (70).** Fresh install → onboarding → account setup; the Google OAuth flow ran through expo-web-browser's custom tab against Supabase and completed, so the device ended up signed in as the owner's account rather than as a guest (the operator did not press the Google button; the owner had the phone in hand). Then Home, Showcase grid, the viewer playing a Motion post on the Qualcomm decoder via the `https://magicbooklet.com/showcase/…` app link, Alerts (push permission left ungranted), Profile with avatar and cover (the expo-image avatar from 5a), and `/pricing`: RevenueCat offerings loaded, "Play Store · Ready", Creator pack at ₹1,950.00 — no purchase made. Cold restart restored the session. 0 logcat signature matches; RevenueCat's info-level "Product not found … Product Type: subs" lines are its second product-type probe for the three one-time packs, which loaded with prices. TOTAL PSS 295 MB, Graphics 126 MB at 600 dpi (the emulator reports no Graphics PSS). Not covered: granting push permission, the image and document pickers, and the Google button artwork itself (the submitting spinner had replaced it by the first capture; the drawable is in the APK per aapt2 and expo-image renders the other bundled assets on this device). The phone was left with the test build installed and signed in; the Play build has to be reinstalled from the store.

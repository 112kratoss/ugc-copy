...
- The editor displays “Remix,” its source title/creator/thumbnail, and accurate Close wording. Prompt resources show Copy prompt and a compact expandable preview; copying always uses the full text. Creator descriptions remain, while the composer's own prompt-card line is dropped in `normalizePostResourceSections` — the one funnel every read and write passes through — so it leaves stored rows on the way out, never reaches a new one, and reads the same on web and mobile whatever app version wrote it. The composer no longer writes it, and the mobile-side filter stays as the last guard for an old app paired with an old backend. Stored rows keep the line until their creator's next edit rewrites them through the write path: a migration to clear them was written and rejected by production, because `post_resource_bundle_revisions` is immutable by trigger — a revision records what a buyer paid for — and writing `post_resource_bundles` would fire `capture_post_resource_bundle_revision` and mint a revision representing no creator action. Reading is the right layer, and every reader including the purchased revision already passes through the normalizer.
...
- Production data, read-only: five bundles and five revisions carry the line, and no creator-written description exists anywhere in either table. The transform that would have cleared them passed as a SELECT and passed on local Postgres — where it matched zero rows, which is exactly why it never reached the immutability trigger that rejected it in production. Verifying a write against a table with no matching rows proves only its syntax.

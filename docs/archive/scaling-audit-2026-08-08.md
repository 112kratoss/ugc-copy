# Historical Scaling Audit — 2026-08-08

> **Archived on 2026-08-22.** This is a frozen engineering journal and evidence
> record. It is not the current scaling source of truth. Its scoped capacity
> certificate applies only to build `c1d494e` and was invalidated immediately by
> later build, schema, route-mix, identity and upload-pipeline changes. Use
> `docs/scaling-audit.md` for current decisions and
> `docs/scaling-certificates/2026-08-10-c1d494e.md` for the extracted historical
> certificate.
...
| F7c | Atomic feed-event batches + mobile offline queue | High | 1/2 | DONE LOCALLY — STORE COVERAGE OPEN | The web batch endpoint performs one service-role-only RPC transaction with per-entry savepoint isolation, mixed outcomes and legacy single-event compatibility. The next mobile train persists and batches non-mutating telemetry with bounded retry, separate signed-in/guest identity buckets and no stored bearer tokens. Installed-base coverage begins only after that train clears review, so the current production certificate remains web-only. |
...
| F10 | Assorted small leaks | Low | 0 | IN PROGRESS — AUTHENTICATED PERF PROOF OPEN | Mobile 404, media grids, webhook budget, feed windowing and encoded/decoded body budgets are implemented. The scheduled harness reports signed-in coverage as NOT CERTIFIED until its optional bot-session secret is configured and produces a passing artifact; the inline-CSS control/candidate A/B also remains release evidence, not a completed measurement. |
...
`scripts/db/schema-fingerprint.sql` also executes successfully against that clean
...
`npm run backfill:showcase-media-cache` (`scripts/backfills/backfill-showcase-media-cache.ts`) walks the bucket, downloads each object and re-uploads it through `storage.update()` with `cacheControl` from the constant. **The write is the purge**; that is the entire point, and re-uploading identical bytes is the mechanism rather than a clumsy way to set metadata.
...
**Status:** IN PROGRESS — authenticated performance proof and inline-CSS A/B open · **Severity:** Low
**Landed:** Mobile 404 pkg 1; studio grid + images pkg 2, 2026-08-08. Webhook budget rides pkg 3. Web feed windowing, native paint containment, idle snapshot writes, a 36-item pure-offset restore cap, decoded-response budgets and raw encoded-response-body budgets landed locally 2026-08-12.
...
- **Web feed DOM growth** — **DONE LOCALLY 2026-08-12.** `/feed` and its embedded home variant retain the complete loaded data set but mount at most 24 measured card subtrees through `WindowedFeedList`, preserving upward scroll with measured absolute offsets rather than discarding history. `content-visibility:auto` plus intrinsic-size fallbacks contain paint work on feed and showcase cards. Both restore caches now serialize on idle with an unmount flush; pure-offset snapshots are capped at 36 items and resume from offset 36, while ranked cursor snapshots are deliberately untouched.
- **Payload weight** — **IN PROGRESS; measurement is implemented, authenticated proof remains open.** The load harness requests `br, gzip, deflate` through Node's raw HTTP client, counts the response body before decompression as `encodedBodyBytes`, then decompresses it itself for `decodedBytes`. Both p95 values have enforceable budgets and are published in JSON, console and GitHub summaries. These encoded bytes are the HTTP entity body as sent (compressed when `Content-Encoding` says so, otherwise identity); they deliberately exclude headers and protocol framing and are never called transferred or wire bytes. A rate-capped production guest sample measured p95 decoded bodies of 644,228 B home, 458,105 B showcase, 485,208 B marketplace, 45,799 B model catalog and 39,900 B feed; decoded budgets retain the next binary-size boundary above roughly 20% headroom (768, 544, 576, 56 and 48 KiB respectively). Encoded ceilings are now enforced as a second, tighter guard but still need a retained control/candidate artifact before changing CSS inlining.

  The edge profile always retains its signed-out targets and declares a separately capped `sort=for-you` signed-in target. The scheduled workflow requires `PERF_AUTH_SUPABASE_URL`, `PERF_AUTH_SUPABASE_ANON_KEY`, `PERF_AUTH_BOT_EMAIL`, and `PERF_AUTH_BOT_PASSWORD`; it exchanges those durable bot credentials through Supabase's password grant for a fresh short-lived user token on every run. Missing credentials, a failed/invalid sign-in, or an oversized auth response exits nonzero before load starts. Credentials and tokens are attached only to the auth request or signed-in target, are never persisted or included in reports, and error messages contain only generic status/context. The target remains capped at 40 load requests so the monitor stays below the endpoint's 60-per-10-minute limiter including warmups. Manual/local runs may still opt into a one-off `PERF_AUTH_BEARER_TOKEN`; ad-hoc runs without auth remain explicitly `not-certified`, but the scheduled job sets `PERF_REQUIRE_SIGNED_IN=1` and cannot pass that way. F10 remains open until a scheduled artifact records the signed-in target passing and the inline-CSS A/B below is attached.

  `experimental.inlineCss` is intentionally still enabled by name in `next.config.ts`. Its A/B is a documented release procedure, not a guess committed into the default configuration:

  1. Create two otherwise identical preview deployments, with `experimental.inlineCss` on for the control and off for the candidate.
  2. Run the edge harness against both previews and retain its per-target encoded-body and decoded-body percentiles. Supply a preview-scoped bot token so the signed-in feed target participates; reject any artifact whose certification status is `not-certified`.
  3. Run the same mobile and desktop Lighthouse profiles on both previews and compare FCP and LCP distributions, not one best run.
  4. Keep CSS inlining unless disabling it saves more than about 150 KiB at p95 and introduces no FCP regression; treat any LCP regression as an additional rejection signal.
  5. Attach both harness reports and Lighthouse artifacts to the release evidence before changing the flag.
...
**Kie publishes no numeric limits at all.** All 20 files in `docs/model-api-references/` document a 429 response; not one states a request rate or a concurrency cap. Two consequences: the numbers below are *ours*, not the provider's, and the reactive half (429 handling, breaker) carries more weight than bucket tuning, because the real ceiling can only be discovered by hitting it. Raise these from the certification load test, not from a hunch.
...
- **Kie does not report per-task cost.** The callback payload carries `status` and nothing else the webhook reads; across all 20 files in `docs/model-api-references/` there is no credits-consumed field, no billing endpoint and no account-usage endpoint — only `402 Insufficient account balance` error rows, and prose pointing at `kie.ai/pricing`. This is the same shape as F14's finding that "reconcile against the provider before refunding" was not buildable: the provider simply does not expose it.
...
The three checks above are each accurate and the conclusion drawn from them was still wrong. "Kie does not report per-task cost" was verified against the **callback payload and the 20 files in `docs/model-api-references/`** — i.e. against the documented API. Nobody looked at the Kie dashboard. It has both halves:
...
`scripts/db/schema-fingerprint.sql` run on both sides through the two routes that
...
| 2026-08-12 | **F10 web feed DOM/snapshot half implemented locally.** The original follow-up plan proposed only `content-visibility`, which does not unmount a DOM subtree and therefore could not honestly close a finding whose premise was “every card mounted.” The implementation adds a measured variable-height window capped at 24 mounted cards, while retaining all loaded data and scroll extent; paint containment remains as a second layer. Feed and showcase snapshots share an extracted idle scheduler, flush on unmount, and pure-offset restore payloads cap at 36 without truncating cursor lanes. F10 remains IN PROGRESS only for decoded payload budgets and the inline-CSS A/B procedure. | Codex |
| 2026-08-12 | **F10 payload budgets completed locally and the finding is DONE.** Successful load samples now expose decoded-byte percentiles, `p95DecodedBytes` participates in validation and pass/fail checks, summaries publish it, and the self-test proves an over-budget payload fails. A bounded production guest sample supplied target-specific ceilings with at least ~20% headroom. The stale line-number citation was replaced with the `experimental.inlineCss` option name, and the control/candidate preview A/B—including compressed bytes and FCP/LCP rejection criteria—is recorded as a repeatable manual release procedure. | Codex |
| 2026-08-12 | **F10 reopened after performance-coverage review.** The previous harness labelled fetch-decoded buffers as `transferredBytes`, did not enforce any pre-decompression body budget, and had no authenticated target, so a guest-only run could not close the finding. The harness now reads the encoded response body directly, decompresses it explicitly, enforces separate encoded/decoded p95 budgets, and removes the misleading transfer metric. A secret-opt-in, rate-capped signed-in `for-you` target is present in the scheduled workflow; if its bot-session token is absent, the artifact explicitly remains NOT CERTIFIED while signed-out targets still run. F10 stays IN PROGRESS until a passing signed-in artifact and the inline-CSS control/candidate evidence exist. | Codex |
| 2026-08-12 | **F7c server half completed locally; mobile exclusion remains.** Multi-event requests now cross the service/database boundary once through `record_showcase_feed_events`; the function is service-role-only, bounded to 25, preserves signed-in versus anonymous delivery ownership, delegates media progress to the existing GREATEST upsert, resolves exact/capped duplicates, and gives poison entries nested-savepoint isolation before applying preference side effects. The byte-identical single-event path remains for installed clients. Proof: clean 177-migration local replay, 44 pgTAP files / 800 assertions, focused TypeScript tests, production build/artifact verification, and a built-server mixed guest batch returning `{recorded:1,rejected:1}` with exactly the valid row durable. | Codex |
| 2026-08-12 | **F7c mobile half completed locally; store coverage remains open.** Impression/open/dwell/media-progress/quick-skip telemetry now enters a versioned AsyncStorage queue, flushes at 10 events, after two seconds, on screen blur/background and on launch restore, and sends chunks of at most 25. Network/429/5xx failures retry with capped jittered backoff; terminal 4xx entries, events older than 24 hours and entries beyond the 200-event cap are discarded. Queue rows capture a non-secret `user:<id>` or `installation:<id>` identity and obtain a fresh token only at flush, with identity revalidation before sending; bearer tokens are never persisted, and a prior user's bucket cannot be sent under a later user. Stateful feedback/save calls remain synchronous singles. Proof: 109 mobile files / 1,006 assertions, typecheck, debug APK build/install on Pixel 9a, guest showcase render against the configured API, and an empty persisted queue after background flush with no token markers. Production RPC and store dispatch are intentionally not performed from this local uncommitted worktree. | Codex |
...
| 2026-08-09 | **Phase 1 is ready for certification — the last blocker was closed by drilling it, not by configuring it.** Source maps now upload (`withSentryConfig`), so stack traces stop pointing at minified code. Every option is a decision rather than a default; the load-bearing one is `deleteSourcemapsAfterUpload: true`, because **this repository and the deployed site are both public** and shipping `.map` files would hand out readable source. A failure mode specific to this project's wiring was closed at the same time: **CI builds without `SENTRY_AUTH_TOKEN`** — it is a Vercel environment variable, not a GitHub secret — **so Vercel's production build is the first one that ever exercises the upload path, after Quality has already gone green.** A wrong-scoped or expired token would surface only there, so an `errorHandler` degrades it to "no source maps", loudly, rather than "no deploy" — the same posture already taken for a missing DSN. Verified against a **clean** `.next`: build and `build:verify` both pass and ffmpeg is still traced into all 8 media routes, which is what this wrapper was most likely to break. **A process note worth keeping:** the first verification run had broken shell redirects, so `build:verify` passed against a *stale* artifact — the same "verified the wrong thing" trap this document keeps recording, caught only by re-reading the command rather than the result. **The second incident recipient is done and, more importantly, drilled:** `magicbooklet` accepted, watches the repository, and a forced watchdog failure opened issue `#46` whose notification **arrived at `info@magicbooklet.com`** — receipt confirmed by the owner, not inferred from configuration. That distinction is the whole item: adding an address or a collaborator looks like it solves this and does not, because GitHub emails scheduled-workflow failures to exactly one person. **Certification is therefore READY.** One caveat carried into it rather than blocking it: IO timing was off for the entire entry baseline, so the run produces IO numbers with no "before" unless a short idle sample is taken first. | Claude Code |

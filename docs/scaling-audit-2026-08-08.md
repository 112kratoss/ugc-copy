...
| F14 | Shared-fate cron; no provider admission control | High | 1 | IN PROGRESS | **Part one landed** except queue-age SLOs — cron split + byte admission, 2026-08-09 (ffmpeg wall-clock kill already existed). **Part two: the money bug is closed** — ambiguous submissions are held, not refunded, 2026-08-09, **verified live** (schema, grants and advisor clean; 0/0 baseline). **Part two's throughput half landed** — token bucket, in-flight cap, `Retry-After` and circuit breaker, 2026-08-09. **Still open**: queue-age SLOs |

...
| F5 | For-you RPC materializes whole catalog | High | 1 | TODO | **Diagnosis confirmed by measurement 2026-08-09** — unlike F5b, execution is real (269 buffers warm vs F5b's 34) and scales with the catalog. Fix targets the right thing; verify against a seeded local catalog, not `pg_stat_statements` |
| F5b | `list_marketplace_resource_bundles` is 47% of RPC time | High | 1 | TODO | **Diagnosis corrected 2026-08-09 — measured, not fixed.** The 1,000 blocks are planning plus a ~200-block PostgREST floor, **not** the query: execution is 34 buffers. The prescribed LIMIT-first fix cannot move today's number, and this item's verify step would misreport a correct fix as a failure. Corrected plan in the section |
...
**Status:** TODO — **diagnosis confirmed by measurement 2026-08-09; no code shipped** · **Severity:** High · **Surface:** migration
...
**Status:** TODO — **diagnosis corrected and measured 2026-08-09; no code shipped** · **Severity:** High · **Surface:** migration + server
...
- **Reduce call volume.** `unstable_cache` (60 s) already fronts this, but only for the unfiltered, `offset = 0` page (`shouldCacheMarketplaceResourceListBasePage`); every tool-filtered, searched or paginated request goes straight through. Its tag set also includes `SHOWCASE_FEED_CACHE_TAG`, so **any showcase feed mutation invalidates the marketplace listing too** — worth confirming that coupling is intended.
- **Drop the 6-arg shim** — still worth doing for the PostgREST overload hazard F6 already hit, though expect no measurable change.
- **Restructure for catalog scale**, verified locally against a seeded catalog rather than against production's 9 rows.

...
|---|---|---|
| Error tracking (web, mobile, server) | a Sentry/PostHog account and its **DSN** — free tier is sufficient. **Account creation is an owner action** and stays so; the wiring is ready to do the moment a DSN exists | env var on Vercel + Expo, then SDK init on all three surfaces |
| ~~Five-minute external monitoring~~ | **OUT OF SCOPE — paid.** The hourly watchdog stands in | — |
| Second incident recipient | **address chosen 2026-08-09: `info@magicbooklet.com`.** But there is no recipient list to add it to — see below | GitHub notification settings, not repo code |
...
So the address is decided — **`info@magicbooklet.com`** (chosen over `athulsmanjeri@gmail.com`, which is almost certainly already the primary; a second recipient on the same mailbox and domain adds no redundancy) — and the remaining work is one of:

| Route | Cost | What it takes |
|---|---|---|
| **Add it as a verified email on the GitHub account** and enable Actions failure notifications for it | free | GitHub → Settings → Emails → add + verify, then Notifications → Actions. **Owner action, ~2 minutes.** Caveat: it is the *same account*, so it survives a lost mailbox but not a lost GitHub login |
| Invite it as a **second GitHub user** with repo access, watching Actions | free | genuinely independent, but needs a second GitHub account |
| Send mail from the workflow directly | **paid** — needs SMTP or a mail API | out of scope per the budget constraint above |

**Recommended: the first route now, the second before the certification run**, since an independent recipient is the entire point of the item and route one does not deliver that.

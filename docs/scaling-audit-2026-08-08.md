...
| F14 | Shared-fate cron; no provider admission control | High | 1 | IN PROGRESS | Cron split — media-heavy jobs isolated into their own function instances, 2026-08-09; admission control, ffmpeg kill, queue SLOs and part two open |
...
**Landed:** Cron shared-fate split — `generation-completions` and `media-preview-repair` now run in their own function instances, 2026-08-09. Media admission control, ffmpeg wall-clock kill, queue-age SLOs and all of part two still open.

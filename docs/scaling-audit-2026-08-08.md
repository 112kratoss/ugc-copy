...
| F14 | Shared-fate cron; no provider admission control | High | 1 | IN PROGRESS | **Part one landed** except queue-age SLOs — cron split + byte admission, 2026-08-09 (ffmpeg wall-clock kill already existed). **Part two open**: token bucket, `Retry-After`, circuit breaker, `submission_unknown`. Not F14 DONE |
...
**Status:** IN PROGRESS · **Severity:** High · **Surface:** server
**Landed:** **Part one**, apart from queue-age SLOs — cron shared-fate split and byte-based media admission, 2026-08-09; the ffmpeg wall-clock kill already existed. **Part two is entirely open** and contains a money bug; read its notes below before starting.
...
**Still open in F14:** queue-age SLOs, and the whole of part two — global provider token bucket, `Retry-After` handling, circuit breaker, and the `submission_unknown` state that reconciles against the provider before refunding.

...
3. A grouped package selector presents Starter, Creator, and Pro as three full-width selectable rows separated by lightweight dividers.
4. Each row prioritizes the credit amount and price. The package name and short usage description are supporting text. Creator keeps a small `Popular` marker.
5. The selected package receives an accent-tinted background, border, and selection indicator. Creator is selected initially because it is the existing popular package.
6. One primary button below the selector states the exact purchase, such as `Buy 2,000 credits for ₹1,660`. It updates when selection changes and shows the existing processing state during purchase.
...
- Use one grouped surface instead of three large standalone cards.
...
- Use a focused selectable-row component inside the pricing screen unless the pattern becomes shared elsewhere.
...
- Unit-test default selection, selected-package resolution, store-price preference, fallback-price formatting, and dynamic purchase button copy.

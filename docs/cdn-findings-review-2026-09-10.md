...
Pre-existing changes were preserved: `docs/scaling-audit.md`, three mobile files (`auth-provider-performance.test.tsx`, `home-side-menu.tsx`, `lib/auth.tsx`), the original CDN report/evidence and the launch-audit document. This review adds only this report and `docs/cdn-audits/2026-09-10-review/`.
...
- [`performance-load-test.mjs:634–637`](../scripts/performance-load-test.mjs#L634) records `cacheControl`, `age` and **Vercel-only** cache status. [`:754`](../scripts/performance-load-test.mjs#L754) rejects any unexpected status only when a target opts in. There is no minimum-hit-ratio assertion, Storage `cf-cache-status` interpretation, Range contract or private cache-directive enforcement here.
- [`performance.yml:40–65`](../.github/workflows/performance.yml#L40) uses the edge profile, two bounded warmups, and a signed-in bot. Warmups are already separated from measured samples ([harness:1055](../scripts/performance-load-test.mjs#L1055)). Results at line 1130 do not capture deployed `/api/app-version` before/after; workflow checkout SHA is not necessarily the live build.

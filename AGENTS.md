...
- **Design**: start at `docs/design/design.md`, which indexes `design-web.md` (web) and `design-mobile.md` (mobile) in the same folder. North star: premium dark AI creator studio — shared tokens, Lucide icons, strict spacing, reusable primitives. Shared web UI lives in `src/app/components/` (not a route). An in-repo design-engineering skill lives at `.agents/skills/emil-design-eng` (symlinked into `.claude/skills/`).

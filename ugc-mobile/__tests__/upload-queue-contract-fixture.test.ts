...
import { runWeightedUploadQueue } from '../lib/upload-file';

/**
 * Replays the shared queue-semantics contract against the MOBILE implementation.
 * The web workspace runs the identical scenarios against its own copy in
 * src/__tests__/upload-queue-contract-fixture.test.ts — the fixture is the
 * single source of truth, so the two deliberate copies cannot drift silently on
 * capacity, video isolation, or failure handling.
 */
...
describe('upload queue shared contract (mobile implementation)', () => {

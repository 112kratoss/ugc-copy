...
describe('FeedVideoPreview backdrop', () => {
  ...
  /** The plain black layer a playing contained video sits on. */
  function blackGrounds(tree: renderer.ReactTestRenderer) {
    return tree.root.findAll((node) => {
      if (String(node.type) !== 'view' || node.props.pointerEvents !== 'none') return false;
      const style = flatStyle(node.props.style);
      return style.position === 'absolute' && style.backgroundColor === mediaColors.mediaGround;
    });
  }

  it('sits a playing contained video on plain black, with no second copy of the poster', () => {
    let tree: renderer.ReactTestRenderer | undefined;
    renderer.act(() => {
      tree = renderer.create(<FeedVideoPreview {...props} />);
    });

    const images = tree!.root.findAll((node) => String(node.type) === 'image');
    // Only the sharp poster: nothing drawn from the thumbhash or blurred behind it.
    expect(images).toHaveLength(1);
    expect(images[0].props.source).toMatchObject({ uri: 'https://cdn.example.com/video-poster.jpg' });
    expect(images.some((node) => node.props.blurRadius !== undefined)).toBe(false);
    expect(blackGrounds(tree!)).toHaveLength(1);
    renderer.act(() => tree!.unmount());
  });

  it('draws nothing behind a video set to videoBackdrop none', () => {
    let tree: renderer.ReactTestRenderer | undefined;
    renderer.act(() => {
      tree = renderer.create(<FeedVideoPreview {...props} videoBackdrop="none" videoContentFit="cover" />);
    });

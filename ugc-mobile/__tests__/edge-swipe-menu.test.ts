...
describe('isLeftEdgeOpenSwipe', () => {
  ...
  it('ignores swipes that start away from the default edge', () => {
    expect(isLeftEdgeOpenSwipe({ x0: 42, dx: 90, dy: 4 })).toBe(false);
    expect(isLeftEdgeOpenSwipe({ x0: 118, dx: 90, dy: 4 })).toBe(false);
  });

  it('honors a custom edge width', () => {
    expect(isLeftEdgeOpenSwipe({ x0: 42, dx: 90, dy: 4, edgeWidth: 64 })).toBe(true);
  });

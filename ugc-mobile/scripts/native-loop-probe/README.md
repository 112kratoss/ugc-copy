...
Use the local CocoaPods Ruby environment if `xcodeproj` is not available. The generator signs with the Apple team named in `LOOP_PROBE_TEAM_ID` (the local Personal Team) for device testing and the separate `com.athuls.magicbooklet.loopprobe` identifier. Install/launch with simctl or devicectl. Read `Documents/results.json` from that app's data container after the label says Complete. Keep simulator and physical results separate; record exact build, device, thermal state and source hash. The fixture has no audio; this probe cannot verify audio continuity.

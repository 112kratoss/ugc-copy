...
/** Each explicit retry obtains a fresh authorized URL and leaves playback paused. */
...
function Attempt({ renew, onRetry, ...props }: Props & { renew: boolean; onRetry: () => void }) {
  ...
  const [status, setStatus] = useState<'loading' | 'ready' | 'error'>(!source || request.mediaType === 'video' ? 'loading' : 'ready');
  ...
  useEffect(() => {
    ...
    if (!initialUrl || request.mediaType === 'video') loading();
    if (!initialUrl) {

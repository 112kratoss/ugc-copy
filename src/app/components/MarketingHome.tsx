...
/**
 * The public marketing homepage, extracted verbatim from `src/app/page.tsx`
 * so it can also serve as the stale-cookie fallback for the signed-in `/`
 * rewrite (`src/app/home/page.tsx`). Keep this module's import graph free of
 * `next/headers` and server auth — `/` must stay statically prerenderable
 * (pinned by marketing-home-page-cache.test.tsx).
 */
...
/**
 * The community strip needs a full row of notes before it earns a section:
 * one or two stale cards in a three-column grid read emptier than no section
 * at all.
 */
const MIN_CREATOR_NOTES = 3;
 ...
 * body — never a media tile. Renders nothing below MIN_CREATOR_NOTES, so the
 * homepage carries no sparse section in a young community.
 */
function HomeCreatorNotes({ data }: { data: Promise<HomePageData> }) {
  const { creatorNotes } = use(data);

  if (creatorNotes.length < MIN_CREATOR_NOTES) {
    return null;
  }

...
import {
  ...
  type TemporaryMediaUploadIntentResult,
} from '@/lib/temporary-media-upload-sign';

type TemporaryMediaUploadSignRouteDependencies = {
...
function createTemporaryMediaUploadSignErrorResponse(
  result: TemporaryMediaUploadIntentResult & { ok: false },
) {
...
async function handleTemporaryMediaUploadSignPOST(
  request: Request,
  ...
  if (!result.ok) {
    return createTemporaryMediaUploadSignErrorResponse(result);
  }

  return NextResponse.json(result.response);
}

export async function postTemporaryMediaUploadSignRouteResponse({
  request,
  dependencies,
}: {
  request: Request;
  dependencies?: TemporaryMediaUploadSignRouteDependencies;
}) {
  const resolvedDependencies = resolveDependencies(dependencies);
  return applyPrivateNoStoreApiResponseHeaders(
    await handleTemporaryMediaUploadSignPOST(request, resolvedDependencies),
    request,
  );
}

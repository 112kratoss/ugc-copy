...
}): Promise<{ removed: boolean; error: { message?: string } | null }> {
  const removablePath = getCanonicalGenerationShowcaseAssetPath(showcaseAssetPath, generationId);
  if (!removablePath) {
    return { removed: false, error: null };
  }
  const result = await adminSupabase.storage.from(SHOWCASE_MEDIA_BUCKET).remove([removablePath]);

-- Report the retention clamp instead of applying it silently.
--
-- 20260809190000_prune_feed_clamp_fact_retention.sql made the fact/event
-- ordering rule clamp upward rather than raise, which was right -- but its
-- summary never actually included the applied value, so a clamped run is
-- indistinguishable from a clean one. Audit Finding B: the verification
-- recipe for the 2026-08-09 retention incident cannot detect the condition
-- it was written for.
--
-- The function is identical except for three new summary fields:
-- `fact_retention_days_requested`, `fact_retention_days_applied`, and
-- `fact_retention_clamped`. feed-maintenance logs a structured warning when
-- the flag is set, and backend health now reports the policy skew directly
-- from the configured constants, so the clamp can never fire unobserved.
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'fact_retention_days_requested', p_fact_retention_days,
    'fact_retention_days_applied', v_fact_retention_days,
